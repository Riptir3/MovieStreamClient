      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-gray-800/50 text-gray-400 uppercase text-xs">
            <th className="p-4">Film ID</th>
            <th className="p-4">Hiba leírása</th>
          ...
          {reports.map((rep) => (
            <tr key={rep.id} className="hover:bg-gray-700/30 transition-colors">
              ...
              <td className="p-4 flex justify-center gap-3">
                <button onClick={() => onApprove(rep)} className="p-2 hover:bg-green-500/20 text-green-400 rounded-lg">
                  <FiCheck size={20} />
                </button>
                <button onClick={() => onReject(rep)} className="p-2 hover:bg-red-500/20 text-red-400 rounded-lg">
                  <FiX size={20} />
                </button>
              </td>
            </tr>
